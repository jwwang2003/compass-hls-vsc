...
// 默认初始化值（根据类型）
const defaultInitializers: Record<string, string> = {
...
function getDefaultValue(declType: string): string {
    for (const key in defaultInitializers) {
        if (declType.split(/\s+/).includes(key)) {
            return defaultInitializers[key];
        }
    }
    return '0';
}
 ...
 * 主处理函数
 */
export function addDefaultInitializers(filePath: string, expandMacros = false): string {
...
if (require.main === module) {
    ...
    const result = addDefaultInitializers(args.source, args.expandMacros);
    if (args.output) {
